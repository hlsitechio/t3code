...
const MODEL_SLUG_SET_BY_PROVIDER: Record<CatalogProvider, ReadonlySet<ModelSlug>> = {
  ...
  "gemini-cli": new Set(MODEL_OPTIONS_BY_PROVIDER["gemini-cli"].map((option) => option.slug)),
  "github-copilot-cli": new Set(
    MODEL_OPTIONS_BY_PROVIDER["github-copilot-cli"].map((option) => option.slug),
  ),
};
